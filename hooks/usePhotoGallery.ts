...
// Numero di foto da caricare per ogni pagina
...
const loadInitialPhotos = useCallback(async () => {
    try {
      ...
      setState(prevState => ({ 
        ...
        isLoading: false,
        page: 1,
        lastAssetId // Salva l'ID dell'ultima foto per la paginazione successiva
      }));
  ...
  // In usePhotoGallery.ts - loadMorePhotos function
const loadMorePhotos = useCallback(async () => {
    ...
    try {
      ...
      // Make sure we don't add duplicates by checking IDs
